...
const BattleRecordItem = ({ record }: BattleRecordItemProps) => {
	...
	return (
		<>
			<div className={`${isWin ? "border-primary-orange" : "border-gray-03"} flex gap-2 rounded-md border-l-8 p-4 bg-gray-05 mb-4 min-h-[10rem] min-w-[50rem]`}>
				<div className="min-w-[11rem]">
					...
					<div className="text-sm">
						<p>
							...
							{!isTeam && isWin && <span className="font-semibold"> / {gameResultMap[record.myTeam.gameResult] ?? record.myTeam.gameResult}</span>}
						</p>
				...
				<div className="grow">
					<p className="mb-3">
						<span className={`text-xs ${record.myTeam.lang === "PY" ? "bg-lang-PY" : "bg-lang-JV"} rounded-lg p-1 font-semibold mr-2`}>{record.myTeam.lang === "PY" ? "PYTHON" : "JAVA"}</span>
						...
						{isTeam ? (
							<div>
								...
								<div className="flex gap-8 mt-4">
									<div className="text-sm">
										<p>실행 시간</p>
										<p className="font-semibold">{record.myTeam.executeTime ? record.myTeam.executeTime + "초" : "-"}</p>
									</div>
									<div className="text-sm">
										<p>메모리</p>
										<p className="font-semibold">{record.myTeam.executeMemory ? record.myTeam.executeMemory + "MB" : "-"}</p>
									</div>
								</div>
							</div>
						) : (
				...
				<div className="bg-gray-06 -mt-4 min-h-[30rem] min-w-[50rem] mb-5 rounded-md py-4 px-6">
								...
								<div className="grow">
									<div>
										<div className="flex justify-between text-sm font-semibold border-b-2 border-gray-03 pb-3">
											<p>
												<span>우리 팀</span>
												<span className={`text-xs ${record.myTeam.lang === "PY" ? "bg-lang-PY" : "bg-lang-JV"} rounded-lg p-1 font-semibold ml-2`}>
										...
										<div>
											{record.myTeam.members.map((member) => (
												<div
													...
													<div>
														<p className={member.userId === Number(userId) ? "text-primary-orange font-semibold" : ""}>{member.nickname}</p>
								...
								<div className="grow">
									<div>
										<div className="flex justify-between text-sm font-semibold border-b-2 border-gray-03 pb-3">
											<p>
												<span>상대 팀</span>
												<span className={`text-xs ${record.opponents[0].lang === "PY" ? "bg-lang-PY" : "bg-lang-JV"} rounded-lg p-1 font-semibold ml-2`}>
							...
							<div>
								{sortedPlayers.map((member, index) => (
									<div
										...
										<p className={`min-w-[4rem] ${member.code !== null ? "text-semibold" : "text-gray-03"} text-sm text-center`}>{member.code !== null ? `${index + 1}등` : "미해결"}</p>
										<div className="flex grow items-center">
											<div className="flex items-center mr-20">
												...
												<div>
													<p className={Number(userId) === member.userId ? "text-primary-orange font-semibold" : ""}>{member.nickname}</p>
													<p className="text-gray-02">현재 순위 {member.rank}위</p>
												</div>
											</div>
											<div className="flex grow justify-between items-center">
												...
												<SmallButton onClick={() => handleCodeClick(member.code, member.nickname)}>코드</SmallButton>
											</div>
										</div>
					...
					<div>
						{isTeam ? (
							...
							<div className="mt-6">
								<p className="bg-gray-05 rounded-md font-semibold text-center px-4 py-2">문제 정보</p>
