  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title='회원가입' height='35rem'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
        <Input name='username' type='text' placeholder='아이디를 입력해 주세요.' onChange={handleChange} value={form.username} className='mt-2' />
        {errors.username && <p className='text-primary-orange text-sm'>{errors.username}</p>}

        <Input name='password' type='password' placeholder='비밀번호를 입력해 주세요.' onChange={handleChange} value={form.password} />
        ...
        {errors.confirmPassword && <p className='text-primary-orange text-sm'>{errors.confirmPassword}</p>}
        {checkPasswordMatch() && <p className={`text-sm ${form.password === form.confirmPassword ? 'text-primary-orange font-semibold' : 'text-gray-03 font-semibold'}`}>{checkPasswordMatch()}</p>}
        ...
        <div>
          <Dropdown options={languageOptions} value={form.mainLang} onChange={handleSelectChange} className='bg-primary-white border rounded-xl' />
        </div>
        ...
        {errors.profileImg && <p className='text-primary-orange text-sm'>{errors.profileImg}</p>}
        {/* 제출중일때 버튼 비활성화 (추후 로딩스피너 추가할 때 수정예정) */}
        <Button
          type='submit'
          className={`w-full h-[2.8rem] text-primary-white ${form.password !== form.confirmPassword || isSubmitting ? 'bg-gray-03 cursor-not-allowed' : 'bg-primary-orange'}`}
